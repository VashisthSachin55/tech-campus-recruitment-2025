...
def extract_logs(log_file, date):
    ...
    try:
        with open(log_file, 'r', encoding='utf-8', errors='ignore') as f, open(output_file, 'w', encoding='utf-8') as out:
            for line in f:
                if line.startswith(date):  # Check if the line starts with the target date
                    out.write(line)
                    found = True
...
if __name__ == "__main__":  # Fixed syntax here
    ...
    log_file_path = sys.argv[1]  # Read log file path from argument
    date_to_search = sys.argv[2]  # Read date from argument
